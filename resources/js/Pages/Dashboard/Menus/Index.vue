    ...
    <div class="flex gap-6 min-h-0">
      ...
      <div class="w-72 shrink-0 flex flex-col gap-4">
        ...
        <div class="space-y-1.5">
          <div v-if="menusLoading" v-for="i in 4" :key="i" class="h-14 bg-gray-100 rounded-xl animate-pulse" />
          <template v-else>
            <button
              v-for="m in menus"
              :key="m.id"
            ...
            >
              <div class="flex items-center justify-between">
                ...
                <span class="text-[10px] font-bold text-white bg-gray-400 rounded-full px-2 py-0.5">
                  {{ m.all_items_count ?? 0 }}
                </span>
              </div>
          ...
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div v-else class="divide-y divide-gray-50">
              <template v-for="item in rootItems" :key="item.id">
                ...
                <!-- Root item row -->
                <div class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 group">
                  <div class="w-1.5 h-5 rounded-full bg-[#0D47A1]/20 shrink-0" />
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-gray-800 truncate">{{ item.label }}</p>
                    <p class="text-xs text-gray-400 font-mono truncate">{{ resolvedUrl(item) }}</p>
                  </div>
                  <span :class="typeBadgeClass(item.type)">{{ item.type }}</span>
                  <span v-if="item.target === '_blank'" class="text-[10px] text-gray-400 hidden group-hover:inline">↗ new tab</span>
                  <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button @click="moveItem(item, 'up')" title="Move up" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors text-xs font-bold">↑</button>
                    <button @click="moveItem(item, 'down')" title="Move down" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors text-xs font-bold">↓</button>
                    <button @click="openItemModal(item)" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-[#0D47A1] hover:bg-blue-50 transition-colors">
                      <Pencil :size="13" />
                    </button>
                    <button @click="confirmDeleteItem(item)" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
                      <Trash2 :size="13" />
                    </button>
                  </div>
                </div>
                ...
                <div
                  ...
                  :key="child.id"
                  class="flex items-center gap-3 pl-12 pr-4 py-3 bg-gray-50/50 hover:bg-gray-100/50 group"
                >
                  <div class="w-4 h-4 border-l-2 border-b-2 border-gray-300 rounded-bl-md shrink-0 -mt-2 mr-1" />
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-700 truncate">{{ child.label }}</p>
                    <p class="text-xs text-gray-400 font-mono truncate">{{ resolvedUrl(child) }}</p>
                  </div>
                  <span :class="typeBadgeClass(child.type)">{{ child.type }}</span>
                  <span v-if="child.target === '_blank'" class="text-[10px] text-gray-400 hidden group-hover:inline">↗ new tab</span>
                  <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button @click="moveItem(child, 'up')" title="Move up" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors text-xs font-bold">↑</button>
                    <button @click="moveItem(child, 'down')" title="Move down" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors text-xs font-bold">↓</button>
                    <button @click="openItemModal(child)" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-[#0D47A1] hover:bg-blue-50 transition-colors">
                      <Pencil :size="13" />
                    </button>
                    <button @click="confirmDeleteItem(child)" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
                      <Trash2 :size="13" />
                    </button>
                  </div>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input v-model="menuForm.name" type="text" placeholder="e.g. Header Navigation"
                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D47A1]/30 focus:border-[#0D47A1]" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
                Location <span class="text-gray-400 normal-case font-normal">(unique key)</span>
              </label>
              <input v-model="menuForm.location" type="text" placeholder="e.g. header"
                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#0D47A1]/30 focus:border-[#0D47A1]"
                :disabled="!!menuModal.editing" />
              <p class="text-xs text-gray-400 mt-1.5 leading-relaxed">
                Suggested:
                <code class="bg-gray-100 px-1.5 py-0.5 rounded">header</code>
                <code class="bg-gray-100 px-1.5 py-0.5 rounded mx-1">footer_quick</code>
                <code class="bg-gray-100 px-1.5 py-0.5 rounded">footer_categories</code>
                <code class="bg-gray-100 px-1.5 py-0.5 rounded ml-1">footer_legal</code>
              </p>
            </div>
          </div>
    ...
    <Transition enter-active-class="transition duration-150" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="itemModal.open" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="itemModal.open = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
          <h3 class="text-lg font-bold text-gray-900 mb-5">{{ itemModal.editing ? 'Edit Item' : 'Add Item' }}</h3>
          <div class="space-y-4">

            <!-- Label -->
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Label</label>
              <input v-model="itemForm.label" type="text" placeholder="e.g. Home"
                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D47A1]/30 focus:border-[#0D47A1]" />
            </div>

            <!-- Link Type tabs -->
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">Link Type</label>
              <div class="flex gap-2">
                <button
                  v-for="t in linkTypes" :key="t.value"
                  @click="itemForm.linkType = t.value"
                  :class="[
                    'flex-1 py-2 text-xs font-semibold rounded-lg border-2 transition-all',
                    itemForm.linkType === t.value
            ...
            <!-- Custom URL -->
            <div v-if="itemForm.linkType === 'custom'">
              <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">URL</label>
              <input v-model="itemForm.url" type="text" placeholder="e.g. / or /category/news or https://…"
                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#0D47A1]/30 focus:border-[#0D47A1]" />
            </div>

            <!-- Page picker -->
            <div v-else-if="itemForm.linkType === 'page'">
              <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Page</label>
              <select v-model="itemForm.page_id"
                class="w-full border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D47A1]/30 focus:border-[#0D47A1] bg-white">
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
                Parent <span class="text-gray-400 normal-case font-normal">(leave blank for top-level)</span>
              </label>
            ...
            <label class="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" v-model="openInNewTab" class="w-4 h-4 accent-[#0D47A1]" />
      ...
      <div v-if="deleteModal.open" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="deleteModal.open = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
          ...
          <h3 class="text-lg font-bold text-gray-900 mb-2">
            Delete {{ deleteModal.type === 'menu' ? 'Menu' : 'Item' }}?
          </h3>
          <p class="text-sm text-gray-500 mb-6">
            <template v-if="deleteModal.type === 'menu'">
              "<strong>{{ deleteModal.target?.name }}</strong>" and all its items will be permanently removed.
            </template>
            <template v-else>
              "<strong>{{ deleteModal.target?.label }}</strong>" will be removed. Child items will also be deleted.
            </template>
          </p>
...
import { Plus, Pencil, Trash2, CheckCircle2, XCircle, Menu as MenuIcon } from 'lucide-vue-next'
...
const typeBadgeClass = (type) => {
  const base = 'text-[10px] font-bold uppercase px-2 py-0.5 rounded-full'
  if (type === 'page')     return `${base} bg-blue-50 text-blue-600`
  if (type === 'category') return `${base} bg-emerald-50 text-emerald-600`
...
const itemForm   = reactive({
  label: '', linkType: 'custom', url: '',
  page_id: null, category_slug: '',
  parent_id: null, target: '_self',
})
...
const rootItems = computed(() =>
  items.value.filter(i => !i.parent_id).slice().sort((a, b) => a.order - b.order)
)
const childrenOf = (parentId) =>
  items.value.filter(i => i.parent_id === parentId).slice().sort((a, b) => a.order - b.order)
...
const saveMenu = async () => {
  if (!menuForm.name.trim() || (!menuModal.editing && !menuForm.location.trim())) return
  ...
  try {
    if (menuModal.editing) {
      ...
      if (idx !== -1) menus.value[idx] = { ...menus.value[idx], name: data.name }
...
const openItemModal = (item = null) => {
  ...
  // Determine linkType from existing item
  let linkType = 'custom'
  if (item) {
    if (item.type === 'page') linkType = 'page'
    else if (item.url && item.url.startsWith('/category/')) linkType = 'category'
  }

  itemForm.label         = item?.label ?? ''
  itemForm.linkType      = linkType
  itemForm.url           = item?.url ?? ''
  itemForm.page_id       = item?.page_id ?? null
  itemForm.category_slug = linkType === 'category' ? (item?.url?.replace('/category/', '') ?? '') : ''
...
const saveItem = async () => {
  if (!itemForm.label.trim()) {
    showToast('Label is required', 'error'); return
  }

  // Resolve URL and DB type based on linkType
  let dbType  = 'custom'
  let url     = null
  let page_id = null

  if (itemForm.linkType === 'custom') {
    url = itemForm.url || '#'
  } else if (itemForm.linkType === 'page') {
    dbType  = 'page'
    page_id = itemForm.page_id
  } else if (itemForm.linkType === 'category') {
    // Store as custom with /category/{slug} URL
    url = itemForm.category_slug ? `/category/${itemForm.category_slug}` : '#'
  }
